...
class Model_2(nn.Module):
    def __init__(self):
        ...
        self.lin1 = nn.Linear(49, 128)  # Relative pos of all voxels of object
...
class Model_Hands(nn.Module):
    def __init__(self):
        super().__init__()

        self.lin1 = nn.Linear(49,256)
        self.lin2 = nn.Linear(256,128)
        self.fc = nn.Linear(128,2) #reshape 168 into 4x42
        #self.fc_to_action_space = nn.Linear(168,2)
        self.relu = nn.LeakyReLU()
        self.tanh = nn.Tanh()
        self.sig = nn.Sigmoid()
...
def select_action_hands(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            # second column on max result is index of where max element was
            # found, so we pick action with the larger expected reward.
            return policy_net_hands(state).max(1).indices.view(1, 1)
    else:
        ...
        for t in count():
          ...
          #  print(pb.getJointState(env.robot_id, env.joint_index["r_moving_tip"]))
           ...
           # targets[2, 2] += .05
            ...
            memory.push(state, torch.tensor(action_select), next_state, torch.tensor(reward))
